...
class PFF_RNN:
    ...
    def __init__(self, args=None, model_dir=None):
        ...
        if theta_r is None: ## representation circuit params
            ...
            self.b3 = tf.Variable(initializer([1, self.n_units]))
            self.b4 = tf.Variable(initializer([1, self.n_units]))
            self.b = tf.Variable(initializer([1, self.n_units]))

            self.W1 = tf.Variable(initializer([self.x_dim, self.n_units]))
            self.W2 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.W3 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.W4 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.W = tf.Variable(initializer([self.n_units, self.n_units]))

            self.V1 = tf.Variable(initializer([self.y_dim, self.n_units]))
            ...
            self.M3 = create_competiion_matrix(self.n_units, n_group=10)
            self.M4 = create_competiion_matrix(self.n_units, n_group=10)
            self.M5 = create_competiion_matrix(self.n_units, n_group=10)
            ...
            self.L3 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.L4 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.L5 = tf.Variable(initializer([self.n_units, self.n_units]))
            
            theta_r = [self.L1,self.L2,self.L3,self.L4,self.L5,
            self.b1,self.b2,self.b3,self.b4,self.b,
            self.W1,self.W2,self.W3,self.W4,self.W,
            self.V1] ## theta_r
        ...
        if theta_g is None: ## generative circuit params
            self.G5 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G4 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G4_1 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G3 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G3_1 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G2 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G2_1 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.G1 = tf.Variable(initializer([self.n_units, self.x_dim]))
            self.G1_1 = tf.Variable(initializer([self.n_units, self.x_dim]))
            theta_g = [self.G5,self.G4,self.G4_1,self.G3,self.G3_1,self.G2,self.G2_1,self.G1,self.G1_1] ## theta_g
        else:
            self.G5 = theta_g[0]
            self.G4 = theta_g[1]
            self.G4_1 = theta_g[2]
    ...
    def forward(self, x):
        ...
        z3 = self.fx(tf.matmul(self.normalize(z2), self.W3) + self.b3)
        z4 = self.fx(tf.matmul(self.normalize(z3), self.W4) + self.b4)
        z5 = self.fx(tf.matmul(self.normalize(z4), self.W) + self.b)
        return [z1,z2,z3,z4,z5] # return all latents
    ...
    def infer(self, x, y, lab, z_lat, K, g_flag, feedback_flag, x_pure, opt=None, g_opt=None, reg_lambda=0.0,
        ...
        if self.rec_gamma > 0.0:
            self.theta = [self.L1,self.L2,self.L3,self.L4,self.L5,
            self.b1,self.b2,self.b3,self.b4,self.b,
            self.W1,self.W2,self.W3,self.W4,self.W,
            self.V1]
        else:
            self.theta = [self.b1,self.b2,self.b3,self.b4,self.b,
            self.W1,self.W2,self.W3,self.W4,self.W,
            self.V1]
        ...
        for k in range(K):
            ...
            if self.gen_gamma > 0.0: ## update generative model
                grad_f = True #False
                Lg, x_hat = self.update_generator(x, y, z_lat, g_opt, feedback_flag, x_pure, reg_lambda=g_reg_lambda,grad_f=grad_f)
            else:
                Lg = 0.0
                x_hat = x * 0

        y_hat = z_lat[len(z_lat)-1]
        return L, y_hat, Lg, x_hat, z_lat
    ...
    def sample(self, n_s=0, z=None, y=None): # samples generative circuit
        ...
        z1 = z[0]
        # generate
        z4_hat = tf.matmul(self.normalize(z5),self.G5) #+ self.c
        z4_bar = self.gfx(z4_hat) #z2 #self.fx(z2 + eps2)

        z3_hat = tf.matmul(self.normalize(z4),self.G4) + tf.matmul(self.normalize(z4_bar),self.G4_1)#+ self.c
        z3_bar = self.gfx(z3_hat) #z2 #self.fx(z2 + eps2)

        z2_hat = tf.matmul(self.normalize(z3),self.G3) + tf.matmul(self.normalize(z3_bar),self.G3_1)#+ self.c
        z2_bar = self.gfx(z2_hat) #z2 #self.fx(z2 + eps2)

        z1_hat = tf.matmul(self.normalize(z2),self.G2) + tf.matmul(self.normalize(z2_bar),self.G2_1)#+ self.c2
        z1_bar = self.gfx(z1_hat)

        z0_hat = self.ofx(tf.matmul(self.normalize(z1),self.G1) + tf.matmul(self.normalize(z1_bar),self.G1_1))#+ self.c1
        
        return z0_hat

    def update_generator(self, x, y, z_lat, opt, feedback_flag, x_pure, reg_lambda=0.0001, grad_f=True):
        '''
        ...
        eps1 = tf.random.normal([half,self.n_units], 0.0, eps_sigma) #* 0
        eps2 = tf.random.normal([half,self.n_units], 0.0, eps_sigma) #* 0
        eps3 = tf.random.normal([half,self.n_units], 0.0, eps_sigma) #* 0
        eps4 = tf.random.normal([half,self.n_units], 0.0, eps_sigma) #* 0
        eps5 = tf.random.normal([half,self.n_units], 0.0, eps_sigma) #* 0
        with tf.GradientTape(persistent=True) as tape:
            z4_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z5 + eps5))),self.G5) #+ self.c
            z4_bar = self.gfx(z4_hat) #z2 #self.fx(z2 + eps2)

            z3_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z4 + eps4))),self.G4) + tf.matmul(tf.stop_gradient(self.normalize(z4_bar)),self.G4_1)#+ self.c
            z3_bar = self.gfx(z3_hat) #z2 #self.fx(z2 + eps2)

            z2_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z3 + eps3))),self.G3) + tf.matmul(tf.stop_gradient(self.normalize(z3_bar)),self.G3_1)#+ self.c2
            z2_bar = self.gfx(z2_hat)

            z1_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z2 + eps2))),self.G2) + tf.matmul(tf.stop_gradient(self.normalize(z2_bar)),self.G2_1)#+ self.c2
            z1_bar = self.gfx(z1_hat)

            z0_hat = self.ofx(tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z1 + eps1))),self.G1) + tf.matmul(tf.stop_gradient(self.normalize(z1_bar)),self.G1_1))#+ self.c2

            e4 = z4_bar - z4#_bar
            L4 = tf.reduce_mean(tf.reduce_sum(tf.math.square(e4),axis=1,keepdims=True))
            e3 = z3_bar - z3#_bar
            L3 = tf.reduce_mean(tf.reduce_sum(tf.math.square(e3),axis=1,keepdims=True))
            e2 = z2_bar - z2#_bar
            ...
            if reg_lambda > 0.0: # weight decay
                reg = (tf.norm(self.G5) + tf.norm(self.G4) + tf.norm(self.G4_1)+ tf.norm(self.G3) + tf.norm(self.G3_1)+ tf.norm(self.G2) +tf.norm(self.G2_1) + tf.norm(self.G1) +tf.norm(self.G1_1)) * reg_lambda
            L = L4 + L3 + L2 + L1 + L0 + reg
        Lg = L4 + L3 + L2 + L1 + L0
        if grad_f == True:
    ...
    def step(self, x, y, lab, z_lat, calc_grad=False, zero_y=False, reg_lambda=0.0):
        ...
        eps3 = tf.random.normal([x.shape[0],self.n_units], 0.0, 1.0) * eps_sigma
        eps4 = tf.random.normal([x.shape[0],self.n_units], 0.0, 1.0) * eps_sigma
        eps5 = tf.random.normal([x.shape[0],self.n_units], 0.0, 1.0) * eps_sigma

        with tf.GradientTape() as tape:
            z1_tm1 = tf.stop_gradient(z_lat[0])
            z2_tm1 = tf.stop_gradient(z_lat[1])
            z3_tm1 = tf.stop_gradient(z_lat[2])
            z4_tm1 = tf.stop_gradient(z_lat[3])
            z5_tm1 = tf.stop_gradient(z_lat[4])

            z1 = tf.matmul(self.normalize(x),self.W1) + tf.matmul(y_,self.V1) + self.b1 + eps1
            if self.rec_gamma > 0.0:
            ...
            z2 = tf.matmul(self.normalize(z1_tm1),self.W2) + self.b2 + eps2
            if self.rec_gamma > 0.0:
            ...
            z3 = tf.matmul(self.normalize(z2_tm1),self.W3) + self.b3 + eps3
            if self.rec_gamma > 0.0:
                L3 = tf.nn.relu(self.L3)
                L3 = L3 * self.M3 * (1. - tf.eye(self.L3.shape[0])) - L3 * tf.eye(self.L3.shape[0])
                z3 = z3 - tf.matmul(z3_tm1, L3) * self.rec_gamma
            z3 = self.fx(z3) * (1.0 - self.alpha) + z3_tm1 * self.alpha

            z4 = tf.matmul(self.normalize(z3_tm1),self.W4) + self.b4 + eps4
            if self.rec_gamma > 0.0:
                L4 = tf.nn.relu(self.L4)
                L4 = L4 * self.M4 * (1. - tf.eye(self.L4.shape[0])) - L4 * tf.eye(self.L4.shape[0])
                z4 = z4 - tf.matmul(z4_tm1, L4) * self.rec_gamma
            z4 = self.fx(z4) * (1.0 - self.alpha) + z4_tm1 * self.alpha

            z5 = tf.matmul(self.normalize(z4_tm1),self.W) + self.b + eps5
            if self.rec_gamma > 0.0:
                L5 = tf.nn.relu(self.L5)
                L5 = L5 * self.M5 * (1. - tf.eye(self.L5.shape[0])) - L5 * tf.eye(self.L5.shape[0])
                z5 = z5 - tf.matmul(z5_tm1, L5) * self.rec_gamma
            z5 = self.fx(z5) * (1.0 - self.alpha) + z5_tm1 * self.alpha

            ## calc loss
            L1 = self.calc_loss(z1, lab, thr=self.thr)
            L2 = self.calc_loss(z2, lab, thr=self.thr)
            L3 = self.calc_loss(z3, lab, thr=self.thr)
            L4 = self.calc_loss(z4, lab, thr=self.thr)
            L5 = self.calc_loss(z5, lab, thr=self.thr)

            reg = 0.0
            if reg_lambda > 0.0: # weight decay
                reg = (tf.norm(self.W1) + tf.norm(self.W2) + tf.norm(self.W3) + tf.norm(self.W4)
                + tf.norm(self.V1)
                + tf.norm(self.W)) * reg_lambda
            L = L5 + L4 + L3 + L2 + L1 + reg
        Lg = L5 + L4 + L3 + L2 + L1
        delta = None
        if calc_grad == True:
            delta = tape.gradient(L, self.theta)
        return [z1,z2,z3,z4,z5], Lg, delta
    ...
    def _step(self, x, y, z_lat, thr=None):
        ...
        z1 = tf.matmul(self.normalize(x),self.W1) + tf.matmul(y_,self.V1) + self.b1
        if self.rec_gamma > 0.0:
            L1 = tf.nn.relu(self.L1)
            L1 = L1 * self.M1 * (1. - tf.eye(self.L1.shape[0])) - L1 * tf.eye(self.L1.shape[0])
            z1 = z1 - tf.matmul(z1_tm1, L1) * self.rec_gamma
        ...
        z2 = tf.matmul(self.normalize(z1_tm1),self.W2) + self.b2
        if self.rec_gamma > 0.0:
            L2 = tf.nn.relu(self.L2)
            L2 = L2 * self.M2 * (1. - tf.eye(self.L2.shape[0])) - L2 * tf.eye(self.L2.shape[0])
            z2 = z2 - tf.matmul(z2_tm1, L2) * self.rec_gamma
        ...
        z3 = tf.matmul(self.normalize(z2_tm1),self.W3) + self.b3
        if self.rec_gamma > 0.0:
            L3 = tf.nn.relu(self.L3)
            L3 = L3 * self.M3 * (1. - tf.eye(self.L3.shape[0])) - L3 * tf.eye(self.L3.shape[0])
            z3 = z3 - tf.matmul(z3_tm1, L3) * self.rec_gamma
        ...
        z4 = tf.matmul(self.normalize(z3_tm1),self.W4) + self.b4
        if self.rec_gamma > 0.0:
            L4 = tf.nn.relu(self.L4)
            L4 = L4 * self.M4 * (1. - tf.eye(self.L4.shape[0])) - L4 * tf.eye(self.L4.shape[0])
            z4 = z4 - tf.matmul(z4_tm1, L4) * self.rec_gamma
        ...
        z5 = tf.matmul(self.normalize(z4_tm1),self.W) + self.b 
        if self.rec_gamma > 0.0:
            L5 = tf.nn.relu(self.L5)
            L5 = L5 * self.M5 * (1. - tf.eye(self.L5.shape[0])) - L5 * tf.eye(self.L5.shape[0])
            z5 = z5 - tf.matmul(z5_tm1, L5) * self.rec_gamma
        ...
        p3, logit3 = self.calc_goodness(z3, thr_)
        p4, logit4 = self.calc_goodness(z4, thr_)
        p5, logit5 = self.calc_goodness(z5, thr_)

        return [z1,z2,z3,z4,z5], [logit1,logit2,logit3,logit4,logit5]
    ...
    def _infer_latent(self, x, y, z_lat):
        ...
        with tf.GradientTape() as tape:
            ...
            z5_bar = self.gfx(self.z_g)
            z5_hat = tf.matmul(self.normalize(z5_bar),self.Gy)
            z4_bar = self.gfx(z4_hat)
            e4 = z4_bar - z4#_bar
            L5 = tf.reduce_mean(tf.reduce_sum(tf.math.square(e4),axis=1,keepdims=True))

        d_z = tape.gradient(L4, self.z_g)
