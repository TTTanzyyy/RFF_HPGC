...
class PFF_RNN:
    def __init__(self, args=None, model_dir=None):
        ...
        if theta_r is None: ## representation circuit params
            ...
            self.V2 = tf.Variable(initializer([self.y_dim, self.n_units])) # inner feedback
            self.W2 = tf.Variable(initializer([self.n_units, self.n_units]))
            self.W = tf.Variable(initializer([self.n_units, self.n_units])) # softmax output
            self.b = tf.Variable(tf.zeros([1, self.n_units]))
            ...
            self.L3 = tf.Variable(initializer([self.n_units, self.n_units])) # lateral lyr 2
            theta_r = [self.L1,self.L2,self.L3,self.b1,self.b2,self.W1,self.W2,self.V2,self.W,self.b] ## theta_r
        else:
            self.M1 = create_competiion_matrix(self.n_units, n_group=10)
            self.M2 = create_competiion_matrix(self.n_units, n_group=10)
            self.M3 = create_competiion_matrix(self.n_units, n_group=10)
        ...
        if theta_g is None: ## generative circuit params
            ...
            theta_g = [self.G3,self.G2,self.G2_1,self.G1,self.G1_1] ## theta_g
        else:
            self.G3 = theta_g[0]
            self.G2 = theta_g[1]
            self.G2_1 = theta_g[2]
    ...
    def save_model(self, model_dir):
        ...
        # checkpoint
        '''
        checkpoint_dict = {f"param_{i}": var for i, var in enumerate(self.theta)}
        checkpoint = tf.train.Checkpoint(**checkpoint_dict)
        save_path = checkpoint.save(model_dir + "RFF_3.ckpt")
        '''
    ...
    def forward(self, x):
        ...
        z3 = self.fx(tf.matmul(self.normalize(z2), self.W) + self.b)
        return [z1,z2,z3] # return all latents
    
    def ff_with_y(self,x,y):
        """
        Forward propagates x y thru rep circuit
        ...
        """
        y_ = y * self.y_scale
        z1 = self.fx(tf.matmul(self.normalize(x), self.W1) + self.b1 + tf.matmul(y_,self.V2))
        z2 = self.fx(tf.matmul(self.normalize(z1), self.W2) + self.b2)
        z3 = self.fx(tf.matmul(self.normalize(z2), self.W) + self.b)
        return [z1,z2,z3] # return all latents
    ...
    def infer(self, x, y, lab, z_lat, K, g_flag, feedback_flag, x_pure, opt=None, g_opt=None, reg_lambda=0.0,
        ...
        if self.rec_gamma > 0.0:
            self.theta = [self.L1,self.L2,self.L3,self.b1,self.b2,self.W1,self.W2,self.V2,self.W,self.b]
        else:
            self.theta = [self.b1,self.b2,self.W1,self.W2,self.V2,self.W,self.b]
    ...
    def update_generator(self, x, y, z_lat, opt, k, feedback_flag, x_pure, reg_lambda=0.0001, grad_f=True):
        ...
        with tf.GradientTape(persistent=True) as tape:
            z2_hat = tf.matmul(self.normalize(self.gfx(z3 + eps3)),self.G3) #+ self.c
            z2_bar = self.gfx(z2_hat) #z2 #self.fx(z2 + eps2)
            ...
            z0_hat = self.ofx(tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z1 + eps1))),self.G1) + tf.matmul(tf.stop_gradient(self.normalize(z1_bar)),self.G1_1))#+ self.c1
            ...
            if reg_lambda > 0.0: # weight decay
                reg = (tf.norm(self.G3) + tf.norm(self.G2) + tf.norm(self.G2_1) + tf.norm(self.G1) + tf.norm(self.G1_1)) * reg_lambda
    ...
    def step(self, x, y, lab, z_lat, calc_grad=False, zero_y=False, reg_lambda=0.0):
        ...
        with tf.GradientTape() as tape:
            ...
            z3 = tf.matmul(self.normalize(z2_tm1),self.W) + self.b + eps3
            if self.rec_gamma > 0.0:
    ...
    def _step(self, x, y, z_lat, thr=None):
        ...
        z3 = tf.matmul(self.normalize(z2_tm1),self.W) + self.b
        if self.rec_gamma > 0.0:
            L3 = tf.nn.relu(self.L3)
            L3 = L3 * self.M3 * (1. - tf.eye(self.L3.shape[0])) - L3 * tf.eye(self.L3.shape[0])
            z3 = z3 - tf.matmul(z3_tm1, L3) * self.rec_gamma
        ...
        return [z1,z2,z3], [logit1,logit2,logit3]
    ...
    def _infer_latent(self, x, y, z_lat):
        ...
        with tf.GradientTape() as tape:
            ...
            z2_hat = tf.matmul(self.normalize(z3_bar),self.G3)
        ...
        z0_hat = self.ofx(self.gfx(tf.matmul(self.normalize(self.gfx(z1)),self.G1)))
        self.z0_hat = z0_hat
