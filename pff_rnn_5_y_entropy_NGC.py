...
class PFF_RNN:
    ...
    def __init__(self, args=None, model_dir=None):
        ...
        if theta_g is None: ## generative circuit params
            ...
            theta_g = [self.G5,self.G4,self.G3,self.G2,self.G1] ## theta_g
        else:
            self.G5 = theta_g[0]
            self.G4 = theta_g[1]
            self.G3 = theta_g[2]
    ...
    def sample(self, n_s=0, z=None, y=None): # samples generative circuit
        ...
        z3_hat = tf.matmul(self.normalize(z4),self.G4)#+ self.c
        z3_bar = self.gfx(z3_hat) #z2 #self.fx(z2 + eps2)

        z2_hat = tf.matmul(self.normalize(z3),self.G3)#+ self.c
        z2_bar = self.gfx(z2_hat) #z2 #self.fx(z2 + eps2)

        z1_hat = tf.matmul(self.normalize(z2),self.G2)#+ self.c2
        z1_bar = self.gfx(z1_hat)

        z0_hat = self.ofx(tf.matmul(self.normalize(z1),self.G1))#+ self.c1
    ...
    def update_generator(self, x, y, z_lat, opt, feedback_flag, x_pure, reg_lambda=0.0001, grad_f=True):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            z3_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z4 + eps4))),self.G4)#+ self.c
            z3_bar = self.gfx(z3_hat) #z2 #self.fx(z2 + eps2)

            z2_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z3 + eps3))),self.G3)#+ self.c2
            z2_bar = self.gfx(z2_hat)

            z1_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z2 + eps2))),self.G2) #+ self.c2
            z1_bar = self.gfx(z1_hat)

            z0_hat = self.ofx(tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z1 + eps1))),self.G1))#+ self.c2
            ...
            if reg_lambda > 0.0: # weight decay
                reg = (tf.norm(self.G5) + tf.norm(self.G4) + tf.norm(self.G3) + tf.norm(self.G2) + tf.norm(self.G1)) * reg_lambda
