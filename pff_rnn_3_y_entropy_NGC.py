...
class PFF_RNN:
    ...
    def __init__(self, args=None, model_dir=None):
        ...
        if theta_g is None: ## generative circuit params
            ...
            theta_g = [self.G3,self.G2,self.G1] ## theta_g
        else:
            self.G3 = theta_g[0]
            self.G2 = theta_g[1]
            self.G1 = theta_g[2]
    ...
    def sample(self, n_s=0, z=None, y=None): # samples generative circuit
        ...
        z1_hat = tf.matmul(self.normalize(z2),self.G2)
    ...
    def update_generator(self, x, y, z_lat, opt, k, feedback_flag, x_pure, reg_lambda=0.0001, grad_f=True):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            z1_hat = tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z2 + eps2))),self.G2)
            z1_bar = self.gfx(z1_hat)

            z0_hat = self.ofx(tf.matmul(tf.stop_gradient(self.normalize(self.gfx(z1 + eps1))),self.G1))
            ...
            if reg_lambda > 0.0: # weight decay
                reg = (tf.norm(self.G3) + tf.norm(self.G2) + tf.norm(self.G1)) * reg_lambda
