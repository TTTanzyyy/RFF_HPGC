...
class PFF_RNN:
    ...
    def __init__(self, args=None, model_dir=None):
        ...
        if theta_r is None: ## representation circuit params
            ...
            self.V2 = tf.Variable(initializer([self.y_dim, self.n_units]))
            ...
            theta_r = [self.L1,self.L2,self.L3,self.L4,
            self.b1,self.b2,self.b3,self.b,
            self.W1,self.W2,self.W3,self.W,
            self.V2] ## theta_r
        else:
            self.M1 = create_competiion_matrix(self.n_units, n_group=10)
            self.M2 = create_competiion_matrix(self.n_units, n_group=10)
        ...
        if theta_g is None: ## generative circuit params
            ...
            theta_g = [self.G4,self.G3,self.G3_1,self.G2,self.G2_1,self.G1,self.G1_1] ## theta_g
        else:
            self.G4 = theta_g[0]
            self.G3 = theta_g[1]
            self.G3_1 = theta_g[2]
    ...
    def forward(self, x):
        ...
        z4 = self.fx(tf.matmul(self.normalize(z3), self.W) + self.b)
        return [z1,z2,z3,z4] # return all latents
    ...
    def infer(self, x, y, lab, z_lat, K, g_flag, feedback_flag, x_pure, opt=None, g_opt=None, reg_lambda=0.0,
        ...
        if self.rec_gamma > 0.0:
            self.theta = [self.L1,self.L2,self.L3,self.L4,
            self.b1,self.b2,self.b3,self.b,
            self.W1,self.W2,self.W3,self.W,
            self.V2] 
        else:
            self.theta = [self.b1,self.b2,self.b3,self.b,
            self.W1,self.W2,self.W3,self.W,
            self.V2] 
        ...
        for k in range(K):
            ...
            if self.gen_gamma > 0.0: ## update generative model
                grad_f = True #False
                Lg, x_hat = self.update_generator(x, y, z_lat, g_opt, k, feedback_flag, x_pure,reg_lambda=g_reg_lambda,grad_f=grad_f)
            else:
                Lg = 0.0
                x_hat = x * 0
    ...
    def update_generator(self, x, y, z_lat, opt, k, feedback_flag, x_pure, reg_lambda=0.0001, grad_f=True):
        '''
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            if reg_lambda > 0.0: # weight decay
                reg = (tf.norm(self.G4) + tf.norm(self.G3) + tf.norm(self.G3_1)+ tf.norm(self.G2) +tf.norm(self.G2_1) + tf.norm(self.G1) +tf.norm(self.G1_1)) * reg_lambda
    ...
    def step(self, x, y, lab, z_lat, calc_grad=False, zero_y=False, reg_lambda=0.0):
        ...
        with tf.GradientTape() as tape:
            ...
            z1 = tf.matmul(self.normalize(x),self.W1) + tf.matmul(y_,self.V2) + self.b1 + eps1
            if self.rec_gamma > 0.0:
            ...
            z4 = tf.matmul(self.normalize(z3_tm1),self.W) + self.b + eps4
            if self.rec_gamma > 0.0:
    ...
    def _step(self, x, y, z_lat, thr=None):
        ...
        z1 = tf.matmul(self.normalize(x),self.W1) + tf.matmul(y_,self.V2) + self.b1
        if self.rec_gamma > 0.0:
            L1 = tf.nn.relu(self.L1)
            L1 = L1 * self.M1 * (1. - tf.eye(self.L1.shape[0])) - L1 * tf.eye(self.L1.shape[0])
            z1 = z1 - tf.matmul(z1_tm1, L1) * self.rec_gamma
        ...
        z4 = tf.matmul(self.normalize(z3_tm1),self.W) + self.b 
        if self.rec_gamma > 0.0:
            L4 = tf.nn.relu(self.L4)
            L4 = L4 * self.M4 * (1. - tf.eye(self.L4.shape[0])) - L4 * tf.eye(self.L4.shape[0])
            z4 = z4 - tf.matmul(z4_tm1, L4) * self.rec_gamma
        ...
        return [z1,z2,z3,z4], [logit1,logit2,logit3,logit4]
    ...
    def _infer_latent(self, x, y, z_lat):
        ...
        with tf.GradientTape() as tape:
            ...
            z4_bar = self.gfx(self.z_g)
            z4_hat = tf.matmul(self.normalize(z4_bar),self.Gy)
            z3_bar = self.gfx(z3_hat)
            e3 = z3_bar - z3#_bar
            L4 = tf.reduce_mean(tf.reduce_sum(tf.math.square(e3),axis=1,keepdims=True))

        d_z = tape.gradient(L3, self.z_g)
